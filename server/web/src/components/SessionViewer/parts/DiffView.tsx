...
function lineDiff(oldText: string, newText: string): DiffRow[] {
  ...
  let j = 0;
  while (i < n && j < m) {
    if (a[i] === b[j]) { rows.push({ tag: "ctx", line: a[i] }); i++; j++; }
    else if (dp[i + 1][j] >= dp[i][j + 1]) { rows.push({ tag: "del", line: a[i] }); i++; }
    else { rows.push({ tag: "add", line: b[j] }); j++; }
  }
    ...
    <div className="diff-block">
      {total > 1 && (
        <div className="diff-block-head">
          <span className="diff-hunk-label">变更 {index + 1}/{total}</span>
          {edit.kind === "replace" && edit.replaceAll && (
            <span className="diff-tag">全部替换</span>
          )}
...
export function DiffView({ data }: { data: NormalizedFileEdit }) {
  const totalStats = useMemo(() => {
    ...
    for (const e of data.edits) {
      if (e.kind === "replace") { const r = lineDiff(e.oldText, e.newText); const s = statsOf(r); added += s.added; removed += s.removed; }
      else if (e.kind === "write") added += e.newText.split("\n").length;
      else removed += e.oldText.split("\n").length;
    }
    return { added, removed };
  }, [data]);

  const variantLabel = VARIANT_LABEL[data.variant] ?? "Edit";

  return (
    <div className="diff-view">
      <div className="diff-header">
        <span className="diff-variant">{variantLabel}</span>
